...
//@Service
@GrpcService
public class SmsNotificationGrpcService
        ...
        try{
            ...
            if(random.nextDouble() < failureRate){
                responseObserver.onError(
                        Status.UNAVAILABLE
                                .withDescription("SMS service temporary unavailable")
                                .asRuntimeException()
                );
                return;
            }

            log.info("SMS sent to user " + request.getUserId() + ": " + request.getMessage());
