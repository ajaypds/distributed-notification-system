...
@Slf4j
@Service
public class EmailNotificationGrpcService
        ...
        try{
            ...
            if(random.nextDouble() < failureRate){
                responseObserver.onError(
                        Status.UNAVAILABLE
                                .withDescription("Email service temporary unavailable")
                                .asRuntimeException()
                );
                return;
            }

            log.info("Email sent to user " + request.getUserId() + ": " + request.getMessage());
